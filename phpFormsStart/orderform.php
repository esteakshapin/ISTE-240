	...
	<div id="container">
		...
		<form name="OrderForm" action="orderprocess.php"  onsubmit="return validateForm();" method="post">
			<p>Name:  <input type="text" name="customerName"  <?php if (isset($_GET['name'])) {
				echo 'value="' . $_GET['name'] . '"';}  ?> /></p>
			<p>ID:  <input type="text"  name="customerID" style="<?php if (isset($_GET['name'])) {
				echo 'background-color: pink';}  ?>" /></p>
			...
			<p> Select the number of toppings you want on your pizza:<br />
				<!-- Note the <label> tags below that allow the user to click on the text! -->
				<input type="radio" name ="toppings" value = "0" id="zero"/><label for="zero">No Extra Toppings<label><br/>
				<input type="radio" name ="toppings" value = "1" id="one"/><label for="zero">One Topping<label><br/>
				<input type="radio" name ="toppings" value = "2" id="two"/><label for="zero">Two Toppings<label><br/>
				<input type="radio" name ="toppings" value = "3" id="three"/><label for="zero">Three Toppings<label><br/>
			</p>
			<p>
